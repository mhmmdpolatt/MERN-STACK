...
import { useComplaintForBlogMutation } from '../../store/apis/userApi';

function ComplaintForm({ onSubmit, setSikayetFormuGetir, sikayetFormuGetir, blogID }) {
  const [description, setDescription] = useState('');
  const [selectedType, setSelectedType] = useState('');
  const [error, setError] = useState('');
  const [complaintForBlog] = useComplaintForBlogMutation();
  ...
  const handleSubmit = async (event) => {
    ...
    console.log(token); // Token'ın doğru olup olmadığını kontrol edin.
    ...
    if (!selectedType || !description) {
      ...
      try {
        ...
        const response = await complaintForBlog({ id: blogID, formData });

        console.log("ŞİKAYET İÇİN RESPONSE", response);
        alert('Şikayet başarıyla oluşturuldu.');
        setSikayetFormuGetir(false);
      } catch (err) {
        console.error(err);
  ...
  return (
    <div className="w-[380px] mx-auto bg-white p-6 rounded-lg shadow-lg">
      ...
      <h4 className="text-sm font-semibold text-gray-800 mb-4 text-center">Gönderiyi Şikayet Ediyorsunuz</h4>
